...
{
  int sum = 0;
  //changed i<=len to i<len to prevent accessing out of bounds array
  for(int i=0; i < len; i++){
    sum += data[i];
  }
  return sum;
...
char* test2_reverse(char* word)
{
  ...
  otherword[len] = '\0'; //null character would be at len 
  delete [] word;
  return otherword;
}
...
int main(int argc, char* argv[])
{
  ...
  if(test == 1){
    ...
    delete [] data; // clears memory allocated to data
    ...
    //delete [] myword; // word was already deleted previously
    delete [] otherword;
  }
  else {
